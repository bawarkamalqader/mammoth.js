...
function DocumentConversion(options, comments) {
    ...
    function convertRun(run, messages, options) {
        ...
        if (run.verticalAlignment === documents.verticalAlignment.superscript) {
            console.log(options);
            var isNote = false;
            for (const child in run.children) {
                if (run.children[child].type == 'noteReference') {
                    isNote = true;
                }
            }
            if (!isNote) {
    ...
    var elementConverters = {
        ...
        "noteReference": function(element, messages, options) {
            ...
            var refTxt = ' ';
            if (documentNotes._notes[noteHtmlId(element)]) {
                var ref = documentNotes._notes[noteHtmlId(element)];
                var children = convertElements(ref.body, messages, options);

                var writer = writers.writer({
                    prettyPrint: options.prettyPrint,
                    outputFormat: options.outputFormat
                });
                Html.write(writer, children);
                refTxt = writer.asString().replace(/(<([^>]+)>)/gi, "");
            }
            var anchor = Html.freshElement("span", {
                // href: "#" + noteHtmlId(element),
                id: 'has-research-reference-text-link',
                class: "popover__content",
                "data-link": " ",
                "data-tippy-content": refTxt
            }, [Html.text(" " + (noteNumber++) + " ")]);
            return [anchor]; // [Html.freshElement("sup", {}, [anchor])];
        },
        "note": function(element, messages, options) {
            ...
            return Html.freshElement("li", {id: noteHtmlId(element)}, children);
        },
